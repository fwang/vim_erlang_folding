...
command! EnableErlangFolds call <SID>SetErlangOptions()
...
if !exists("*GetErlangFold")
	...
	function GetErlangFold(lnum)
		...
		" End fold: last line of func and next line is not empty
		if line =~ s:ErlangFunEnd
			let nextline = getline(lnum + 1)
			if ! (nextline =~ s:ErlangBlankLine)
				return '<1'
			endif
		endif

		" End fold: empty line and previous line is end of func
		if line =~ s:ErlangBlankLine && prevline =~ s:ErlangFunEnd
			return '<1'
		endif
